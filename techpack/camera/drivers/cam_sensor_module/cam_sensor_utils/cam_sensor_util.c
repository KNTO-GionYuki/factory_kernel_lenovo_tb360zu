		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CUSTOM_REG2:
			...
			break;
#ifdef CONFIG_LDO_WL2868C
		case SENSOR_WL2868C_LDO1:
		case SENSOR_WL2868C_LDO2:
		case SENSOR_WL2868C_LDO3:
		case SENSOR_WL2868C_LDO4:
		case SENSOR_WL2868C_LDO5:
		case SENSOR_WL2868C_LDO6:
		case SENSOR_WL2868C_LDO7:
		case SENSOR_WL2868C_LDO1_1P1:
			//wl2868C out port num :
			//              OUT_LDO1 = 0
			//		 ------
			//              OUT_LDO7 = 6
			//but we pre set SENSOR_WL2868C_LDO1 = 13.
			wl2866_iotype = ((int)power_setting->seq_type) - 13;
			if ((wl2866d_camera_power_up(wl2866_iotype) < 0) && (et5907_camera_power_up(wl2866_iotype) < 0)) {
				CAM_ERR(CAM_SENSOR, "camera_power_up_io_type [%d] failed", power_setting->seq_type);
				rc = -1;
				goto power_up_failed;
			}
			...
			CAM_INFO(CAM_SENSOR, "power_setting->seq_type = [%d]", power_setting->seq_type);
			break;
#endif
		default:
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_WL2868C_LDO1_1P1:
			...
			if ((wl2866d_camera_power_down(wl2866_iotype) < 0)  && (et5907_camera_power_down(wl2866_iotype) < 0)) {
				CAM_ERR(CAM_SENSOR, "camera_power_down iotype [%d] failed", pd->seq_type);
				break;
			}
			...
			usleep_range(wl2866_time_delay , wl2866_time_delay + 100);*/
			CAM_INFO(CAM_SENSOR, "wl2866d_iotype = [%d]", pd->seq_type);
			break;
#endif
		default:
			CAM_ERR(CAM_SENSOR, "error power seq type %d",
