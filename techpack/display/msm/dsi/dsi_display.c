...
/*Linden code for JLINDEN-288 by yangjm11 at 20221119 start*/
#ifdef CONFIG_HQ_SYSFS_SUPPORT
#include <linux/hqsysfs.h>
#endif
/*Linden code for JLINDEN-288 by yangjm11 at 20221119 end*/
...
static char dsi_display_panel_det[MAX_CMDLINE_PARAM_LEN];
...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	int i;
	struct dsi_display_ctrl *ctrl;
        char *boot_panel_str = NULL;
	boot_panel_str = dsi_display_panel_det;
	...
	/*Linden code for JLINDEN-288 by yangjm11 at 20221119 start*/
		...
		/* linden code for JLINDEN-1640 add by yangjinmin at 20230203 start*/
                /* linden code for JLINDEN-3226 add by yangjinmin at 20230225 start*/
                /* linden code for JLINDEN-10149 add by zhengjie6 at 20230522 start*/
		printk("[LCD_LOG] %s", (char *)boot_panel_str);
		if (strcmp("no_det_panel", dsi_display_panel_det)) {
		   if (!strcmp((char *)display->panel->name, "hx83102 amoled video mode dsi visionox panel with DSC")) {
			hq_register_hw_info(HWID_LCM,"hx83102 amoled video mode panel with DSC(1200*2000)");
			printk("(char *)display->panel->name %s ===%d\n", (char *)display->panel->name,__LINE__);
		   }else if (!strcmp((char *)display->panel->name, "nt36523n amoled video mode dsi visionox panel with DSC")) {
			hq_register_hw_info(HWID_LCM,"nt36523n amoled video mode panel with DSC(1200*2000)");
			printk("(char *)display->panel->name %s ===%d\n", (char *)display->panel->name,__LINE__);
		   }else if (!strcmp((char *)display->panel->name, "hx83102 tg amoled video mode dsi visionox panel with DSC")) {
			hq_register_hw_info(HWID_LCM,"hx83102 tg amoled video mode panel with DSC(1200*2000)");
			printk("(char *)display->panel->name %s ===%d\n", (char *)display->panel->name,__LINE__);
		   }else if (!strcmp((char *)display->panel->name, "hx83102 gx amoled video mode dsi visionox panel with DSC")) {
			hq_register_hw_info(HWID_LCM,"hx83102 gx amoled video mode panel with DSC(1200*2000)");
			printk("(char *)display->panel->name %s ===%d\n", (char *)display->panel->name,__LINE__);
		   }
		} else {
		   printk("(char *)display->panel->name %s ===%d\n", (char *)display->panel->name,__LINE__);
		}
                /* linden code for JLINDEN-10149 add by zhengjie6 at 20230522 end*/
                /* linden code for JLINDEN-3226 add by yangjinmin at 20230225 end*/
		/* linden code for JLINDEN-1640 add by yangjinmin at 20230203 end*/
	#endif
	}
	/*Linden code for JLINDEN-288 by yangjm11 at 20221119 end*/
...
int dsi_display_prepare(struct dsi_display *display)
{
	...
	/*linden code for JLINDEN-396 by yangjinmin at 20221206 start*/
       if (!(mode->dsi_mode_flags & DSI_MODE_FLAG_POMS) &&
               (!is_skip_op_required(display))) {
               dsi_panel_power_on_vddio_mipi_lp11(display->panel);
               mdelay(2);
       }
...
/* linden code for JLINDEN-3226 add by yangjinmin at 20230225 start*/
module_param_string(panel_det_flag, dsi_display_panel_det, MAX_CMDLINE_PARAM_LEN, 0600);
MODULE_PARM_DESC(panel_det_flag,
	"msm_drm.panel_det_flag=<display node>:<configX> where <display node> is 'primary dsi display node name' and <configX> where x represents index in the topology list");
/* linden code for JLINDEN-3226 add by yangjinmin at 20230225 end*/
