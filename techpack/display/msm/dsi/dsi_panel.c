...
/*Linden & ebony code for OEBONYSB-547 by xiepeng at 20221109 start*/
bool backlic_set = false;
int select_panel = -1;
EXPORT_SYMBOL(select_panel);
/*linden code for JLINDEN-222 by yangjinmin at 20221130 start*/
extern int sgm37604ayg_bl_enable(int en);
extern int sgm37604ayg_set_bl(unsigned int bl_lv);
extern void sm5109_bias_enable(int en);
extern void sm5109_bias_disable(int en);
extern void sm5109_bias_set(void);
/*linden code for JLINDEN-222 by yangjinmin at 20221130 end*/
/*Linden & ebony code for OEBONYSB-547 by xiepeng at 20221109 end*/

/*linden code for JLINDEN-396 by yangjinmin at 20221206 start*/
extern void tp_rst_gpio_enable(int en);
...
int select_tp_fw_mode = 0;
EXPORT_SYMBOL(select_tp_fw_mode);
...
/*linden code for JLINDEN-396 by yangjinmin at 20221206 start*/
int dsi_panel_power_on_vddio_mipi_lp11(struct dsi_panel *panel)
{
       int rc = 0;
       if(gesture_enabled == false) {
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	/*linden code for JLINDEN-396 by yangjinmin at 20221206 start*/
	tp_rst_gpio_enable(0);
    /*linden code for JLINDEN-9783 by zhengjie6 at 20230524 start*/
	if(gesture_enabled == false) {
		sm5109_bias_disable(0);
	}
    /*linden code for JLINDEN-9783 by zhengjie6 at 20230524 end*/
#if 0
	rc = dsi_pwr_enable_regulator(&panel->power_info, true);
	if (rc) {
		DSI_ERR("[%s] failed to enable vregs, rc=%d\n",
	...
	/*linden code for JLINDEN-396 by yangjinmin at 20221206 start*/
	if(select_panel)
		tp_rst_gpio_enable(1);
	mdelay(2);
	sm5109_bias_set();
	sm5109_bias_enable(1);
	mdelay(4);
	if(!select_panel){
		/*linden code for JLINDEN-482 by yangjinmin at 20221209 start*/
		mdelay(10);/*nt36523n more than 10ms */
		/*linden code for JLINDEN-482 by yangjinmin at 20221209 end*/
	}
	/*linden code for JLINDEN-396 by yangjinmin at 20221206 end*/
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if(gesture_enabled == false) {
		/*linden code for JLINDEN-482 by yangjinmin at 20221209 start*/
		if(!select_panel){
			mdelay(90);/*nt36523n more than 90ms */
		}
		/*linden code for JLINDEN-482 by yangjinmin at 20221209 end*/
				...
				enum dsi_cmd_set_type type)
{
	...
	/* linden code for JLINDEN-1299 add by yangjinmin at 20230130 start*/
	if(select_panel){
		DSI_ERR("[LCD_LOG] Remove the PPS settings of the platform to the main supply screen\n");
		if(type == DSI_CMD_SET_POST_ON)
			return rc;
		if(type == DSI_CMD_SET_PPS)
			return rc;
	}
	/* linden code for JLINDEN-1299 add by yangjinmin at 20230130 end*/
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	/*linden code for JLINDEN-516 by yangjinmin at 20221215 start*/
#ifdef LCM_PRC
	DSI_ERR("lcm enter prc");
	if(!select_panel){
        	bl_lvl = bl_lvl*720;
        	bl_lvl /= 1000;
			DSI_ERR("lcm prc nova");
	}else{
        	bl_lvl = bl_lvl*925;
        	bl_lvl /= 1000;
	}
#else
	DSI_ERR("lcm enter row");
	if(!select_panel){
        	bl_lvl = bl_lvl*640;
        	bl_lvl /= 1000;
			DSI_ERR("lcm row nova");
	}else{
        	bl_lvl = bl_lvl*925;
        	bl_lvl /= 1000;
	}
#endif
	/*linden code for JLINDEN-516 by yangjinmin at 20221215 end*/
	...
	/*Linden & ebony code for OEBONYSB-547 by xiepeng at 20221109 start*/
		if(!backlic_set){
		DSI_ERR("update sgm37604ayg enable backlight :%d\n", bl_lvl);
		sgm37604ayg_bl_enable(1);
		backlic_set = true;
	}
	sgm37604ayg_set_bl(bl_lvl);
	/*Linden & ebony code for OEBONYSB-547 by xiepeng at 20221109 end*/
		break;
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	/*Linden & ebony code for OEBONYSB-547 by xiepeng at 20221109 start*/
	/*Linden code for OEBONYSB-10149 by zhengjie6 at 20230522 start*/
	if (!strcmp(panel->name, "nt36523n amoled video mode dsi visionox panel with DSC")) {
		select_panel = 0;
		printk("panel->name =  %s,select_panel = %d, %d\n",panel->name,select_panel,__LINE__);
	} else {
		select_panel = 1;
		printk("panel->name =  %s,select_panel = %d, %d\n",panel->name,select_panel,__LINE__);
	}
	/*Linden code for OEBONYSB-10149 by zhengjie6 at 20230522 end*/
    /*Linden & ebony code for OEBONYSB-547 by xiepeng at 20221109 end*/

/* Linden code for JLINDEN-10584 by dingying at 20230607 start */
	if (!strcmp(panel->name, "hx83102 amoled video mode dsi visionox panel with DSC")) {
		select_tp_fw_mode = TP_BOE_HIMIX;
	} else if (!strcmp(panel->name, "nt36523n amoled video mode dsi visionox panel with DSC")){
		select_tp_fw_mode = TP_BOE_NOVA;
	} else if (!strcmp(panel->name, "hx83102 tg amoled video mode dsi visionox panel with DSC")){
		select_tp_fw_mode = TP_BOE_HIMIX_TG;
	} else if (!strcmp(panel->name, "hx83102 gx amoled video mode dsi visionox panel with DSC")) {
		select_tp_fw_mode = TP_BOE_HIMIX_GX;
	} else {
		DSI_ERR("unmatched panel name\n");
	}
/* Linden code for JLINDEN-10584 by dingying at 20230607 end */
